...
def load_definition_pipelines(model_ids, gpu_ids):
    check_cuda_gpus(required_gpus=max(gpu_ids) + 1)
    return {name: load_model(model_id, gpu_id) for name, model_id, gpu_id in zip(MODEL_NAMES, model_ids, gpu_ids)}
...
def define_candidates(definitions_df, generators, batch_size, output_path):
...
def main():
    ...
    generators = load_definition_pipelines(args.model_ids, parse_gpu_ids(args.gpu_ids))
    definitions_df = define_candidates(definitions_df, generators, args.batch_size, definitions_output)
