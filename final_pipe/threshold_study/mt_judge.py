...
def round_trip_sentence_all_languages(generator, sentence, batch_size):
    ...
    forward_prompts = [
        (
            f"Translate the following sentence into {language_name}. "
            f"The source language may be any language. "
    ...
    backward_prompts = [
        (
            f"Translate the following {language_name} sentence back into the original source language. "
            f"Return only the translation.\n\n"
            f"Original sentence: {sentence}\n"
            f"{language_name} translation: {translated_sentence}"
        )
        for (_, language_name), translated_sentence in zip(LANGUAGES, translated_sentences)
    ]

    back_to_original_sentences = ask_llama_batch(generator, backward_prompts, batch_size)

    return {
        language_key: {
            "translated_sentence": translated_sentence,
            "back_to_original_sentence": back_to_original,
        }
        for (language_key, _), translated_sentence, back_to_original in zip(
            LANGUAGES,
            translated_sentences,
            back_to_original_sentences,
        )
    }
...
def score_sentence_words(row, generator, fuzzy_threshold, batch_size, nlp):
    ...
    back_word_sets_by_language = {
        language_key: normalized_word_set(result["back_to_original_sentence"])
        for language_key, result in language_results.items()
    }
    back_words_by_language = {
        language_key: [
            normalize_word(word)
            for word in tokenize_words(result["back_to_original_sentence"])
    ...
    for word_index, original_word in enumerate(original_words):
        ...
        for language_key, _ in LANGUAGES:
            ...
            output_row[f"{language_key}_back_to_english_sentence"] = language_results[language_key]["back_to_original_sentence"]
