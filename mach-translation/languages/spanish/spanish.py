...
def round_trip_sentence(generator, sentence):
    ...
    spanish_sentence = ask_llama(
        generator,
        f"Translate this English sentence to Spanish: {sentence}",
    )

    back_to_english = ask_llama(
        generator,
        f"Translate this Spanish sentence to English: {spanish_sentence}",
    )

    return spanish_sentence, back_to_english
...
def score_sentence_words(row, generator, fuzzy_threshold, nlp):
    ...
    # Machine translation
    spanish_sentence, back_to_english = round_trip_sentence(generator, sentence)
    ...
    spanish_word_set = normalized_word_set(spanish_sentence)
    ...
    for word_index, original_word in enumerate(original_words):
        ...
        term_failed_to_translate = (
            ...
            and candidate_norm in spanish_word_set
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Detect slang candidates using full-sentence English-Spanish-English round-trip translation."
    )
